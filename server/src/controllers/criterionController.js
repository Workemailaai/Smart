...
class CriterionController {
  /** Создание критерия организатором */
  static async createCriterion(req, res, next) {
    try {
      ...
      return res
        .status(201)
        .json(formatResponse(201, "Criterion created", criterion));
    } catch (error) {
      return next(error);
    }
  }

  /** Список критериев конкурса */
  static async getCriteria(req, res, next) {
    try {
      const criteria = await CriterionService.listCriteria(req.params.contestId);
      return res
        .status(200)
        .json(formatResponse(200, "Criteria list", criteria));
    } catch (error) {
      return next(error);
    }
  }
